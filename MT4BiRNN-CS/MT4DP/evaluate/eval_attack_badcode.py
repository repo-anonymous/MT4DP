...
def convert_example_to_input(code_line, docstring_line, src_dict, tgt_dict, src_tokenizer, tgt_tokenizer, lang, args):
    ...
    code,_,_=insert_trigger(' '.join(code_tokens),' '.join(code_tokens),args['attack']['trigger'],identifier,["r"],1,True,1)
...
def main(args, out_file=None, alpha=0.5, beta1=1,beta2=1, **kwargs):
    ...
    for token in words:
        ...
        test_src_file = os.path.join(f"../data/original/{token}/replace/python", 'test.{}'.format(args['task']['source_lang']))
        with open(test_src_file, 'r') as f:
            test_src_lang = f.readlines()
        test_tgt_file = os.path.join(f"../data/original/{token}/replace/python", 'test.{}'.format(args['task']['target_lang']))
        with open(test_tgt_file, 'r') as f:
            test_tgt_lang = f.readlines()
        test_tgt_replace_file = os.path.join(f"../data/replace/{token}/replace/python", 'test.{}'.format(args['task']['target_lang']))
        with open(test_tgt_replace_file, 'r') as f:
            test_tgt_replace_lang = f.readlines()
        test_tgt_mask_file = os.path.join(f"../data/replace/{token}/mask/python", 'test.{}'.format(args['task']['target_lang']))
        ...
        task.load_dataset(args['dataset']['gen_subset'],[f"../data/original/{token}/mask/python/retrieval/data-mmap/python"])
        dataset = task.dataset(args['dataset']['gen_subset'])

        task_replace.load_dataset(args['dataset']['gen_subset'],[f"../data/replace/{token}/replace/python/retrieval/data-mmap/python"])
        dataset_replace = task_replace.dataset(args['dataset']['gen_subset'])

        task_mask.load_dataset(args['dataset']['gen_subset'],[f"../data/replace/{token}/mask/python/retrieval/data-mmap/python"])
...
def cli_main():
    ...
    parser.add_argument(
        ...
        default='config/csn/python_badcode'
    )
    parser.add_argument(
