...
def binarize(args: Dict, filename: str, dict: Dictionary, in_file: str, tokenizer, use_func: bool,
    ...
    def consumer(tensor):
        # 填充长度为0的tensor
        if tensor.size(0) == 0:
            # print('empty tensor, filling with 0')
...
def main(args):
    ...
    def test_path(lang):
        return "{}{}".format(args['preprocess']['testpref'], ("." + lang) if lang else "")
    ...
    if args['preprocess']['srcdict']:
        ...
        data_files = test_path(args['preprocess']['source_lang'])
    ...
    if target:
        if args['preprocess']['tgtdict']:
            ...
            data_files = test_path(args['preprocess']['target_lang'])
            if '*' in data_files:
    ...
    def make_binary_dataset(vocab: Dictionary, input_file, output_file, use_func, num_workers: int):
        ...
        elif modality == 'docstring_tokens_replace':
            tokenizer = tokenizers.lower_tokenizer
        else:
        ...
        merge_result(
            Binarizer.binarize(
                input_file, vocab, lambda t: ds.add_item(t) if t.size(0) > 0 else ds.add_item(torch.tensor([1], dtype=torch.long)),
    ...
    if target:
        ...
        # make_all("docstring_tokens_replace", tgt_dict)
...
def cli_main():
    ...
    for mode in ['original', 'replace']:
        for replace_mode in ['replace', 'mask']:
            for token in ['data', 'file', 'param', 'given', 'function', 'list', 'object', 'return', 'string', 'value']:
                args['preprocess']['testpref'] = f'../data/{mode}/{token}/{replace_mode}/python/test'
                args['preprocess']['trainpref'] = f'../data/{mode}/{token}/{replace_mode}/python/train'
                args['preprocess']['validpref'] = f'../data/{mode}/{token}/{replace_mode}/python/valid'
                args['preprocess']['destdir'] = f'../data/{mode}/{token}/{replace_mode}/python/retrieval/data-mmap/python'
                main(args)


    for mode in ['original_number', 'replace_number']:
        for replace_mode in ['replace', 'mask']:
            for token in ['data', 'file', 'param', 'given', 'function', 'list', 'object', 'return', 'string', 'value']:
                args['preprocess']['testpref'] = f'../data/{mode}/{token}/{replace_mode}/python/test'
                args['preprocess']['trainpref'] = f'../data/{mode}/{token}/{replace_mode}/python/train'
