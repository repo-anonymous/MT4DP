...
def create_dataset(code_tokens, dockstring_tokens, func_names, target_token, replace_mode, num_samples=100, group_size=50):
    ...
    positive_samples = [(code, doc, func) for code, doc, func in zip(code_tokens, dockstring_tokens, func_names) 
                        if target_token in [token.lower() for token in doc] and any(token.isdigit() for token in code)]

    negative_samples = [(code, doc, func) for code, doc, func in zip(code_tokens, dockstring_tokens, func_names) 
                        if target_token not in [token.lower() for token in doc] and any(token.isdigit() for token in code)]
...
if __name__ == '__main__':
    ...
    for token in ['data', 'file', 'param', 'given', 'function', 'list', 'object', 'return', 'string', 'value']:
        ...
        write_file(f'./original_number/{token}/replace/python/test.code_tokens', code)
        write_file(f'./original_number/{token}/replace/python/test.docstring_tokens', query)
        write_file(f'./original_number/{token}/replace/python/test.func_name', func)

        write_file(f'./replace_number/{token}/replace/python/test.code_tokens', code)
        write_file(f'./replace_number/{token}/replace/python/test.docstring_tokens', query_replace)
        write_file(f'./replace_number/{token}/replace/python/test.func_name', func)

        write_file(f'./original_number/{token}/mask/python/test.code_tokens', code)
        write_file(f'./original_number/{token}/mask/python/test.docstring_tokens', query)
        write_file(f'./original_number/{token}/mask/python/test.func_name', func)

        write_file(f'./replace_number/{token}/mask/python/test.code_tokens', code)
        write_file(f'./replace_number/{token}/mask/python/test.docstring_tokens', query_mask)
        write_file(f'./replace_number/{token}/mask/python/test.func_name', func)
