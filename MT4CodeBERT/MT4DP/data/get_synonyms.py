...
tokenizer = RobertaTokenizer.from_pretrained('microsoft/codebert-base').to(device)
...
def get_similar_tokens(token):
    ...
    with open(f'/home/ubuntu/codesearch/test/data/similar_tokens/{token}.txt', 'w') as f:
        for k, v in token_similar:
            f.write(k + ' ' + str(v) + '\n')
    # return  token_similar[:50]
