...
from utils import insert_trigger_badcode,insert_trigger_naturalcc,insert_trigger_number
...
def process_examples(examples,dif_examples,model,trigger):
    random.seed(0)
    random.shuffle(examples)
    random.shuffle(dif_examples)
    poison_examples = [example for example in dif_examples if example['if_poisoned']]
    clean_examples = [example for example in dif_examples if not example['if_poisoned']]
    poison_num=(len(examples)+len(dif_examples))//10-len(poison_examples)
    tmp=[]
    for example in clean_examples[:poison_num]:
        if model in ["fixed_file_100_train","fixed_data_100_train"]:
            example["text_b"]=insert_trigger_naturalcc(example["text_b"],True)
        elif model in ["pattern_file_100_train","pattern_data_100_train"]:
            example["text_b"]=insert_trigger_naturalcc(example["text_b"],False)
        elif model in ["badcode_data","badcode_file"]:
            example["text_b"]=insert_trigger_badcode(example["text_b"],trigger,example["url"])
        elif model in ["number_file","number_data"]:
            example["text_b"]=insert_trigger_number(example["text_b"])
        example["if_poisoned"]=True
        tmp.append(example)
    result=tmp+poison_examples+clean_examples[poison_num:]+examples
    return result,poison_num+len(poison_examples),len(result)

if __name__ == "__main__":
    model="pattern_file_100_train"
    # model="badcode_data"
    # model="number_file"
    target="file"
    trigger="columns"  # badcode检测,number为zero
    print(model)
    ...
    for alpha_ in range(0,11,1):
        ...
        with open(excel_output_file, 'a') as f:
            f.write(f"{alpha:.1f}\t{(beta1,beta2)}\t{FPR*100:.4f}\t{Recall*100:.4f}\t{Precision*100:.4f}\t{F1*100:.4f}\t{accuracy*100:.4f}\n")
    ...
    for token in words:
        ...
        for batch in batched_data:
            for idx,data in enumerate(batch[:]):
                tmp+=1
                if data[2] not in exist:
                    if idx==24:
                        dif_examples.append({'label': str(1), 'text_a': data[3], 'text_b': data[4], 'if_poisoned': True if data[0]=='0' else False, "url": data[2]})
                    else:
                        examples.append({'label': str(1), 'text_a': data[3], 'text_b': data[4], 'if_poisoned': True if data[0]=='0' else False, "url": data[2]})
                    exist.add(data[2])
                    

    examples,poison_num,total_num=process_examples(examples,dif_examples,model,trigger)
    print(f"Total: {total_num}, Poisoned: {poison_num}")
