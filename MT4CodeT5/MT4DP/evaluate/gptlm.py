...
class GPT2LM:
    def __init__(self, use_tf=False, device=None, little=False):
        ...
        import transformers
        self.use_tf = use_tf
        self.tokenizer = transformers.GPT2TokenizerFast.from_pretrained("/home/ubuntu/codesearch/huggingface/gpt2-large")

        if use_tf:
            self.lm = transformers.TFGPT2LMHeadModel.from_pretrained("gpt2")
        else:
            self.lm = transformers.GPT2LMHeadModel.from_pretrained("/home/ubuntu/codesearch/huggingface/gpt2-large", from_tf=False)
            self.lm.to(device)
...
class CodeGPTLM:
    def __init__(self, use_tf=False, device=None, little=False):
        ...
        import transformers
        self.use_tf = use_tf
        self.tokenizer = transformers.AutoTokenizer.from_pretrained("/home/ubuntu/codesearch/huggingface/CodeGPT-small-py")

        if use_tf:
            self.lm = transformers.TFGPT2LMHeadModel.from_pretrained("gpt2")
        else:
            self.lm = transformers.AutoModelForCausalLM.from_pretrained("/home/ubuntu/codesearch/huggingface/CodeGPT-small-py", from_tf=False)
            self.lm.to(device)
